...
fup3 <- read.csv(file.path(dropbox,
                'Rwanda Roads Data/Primary data/HH survey/endline/data/raw/Household_survey_2023_WIDE.csv'),
                     stringsAsFactors = FALSE)


  # Variables submissiondate, starttime, endtime are recorded in dmy hms hence treated accordingly
date_variables_dmy <- fup3 %>%
...
#   
  ...
  ## 1. Overall Construction ----
    ...
    hfc_constr <- fup3 #%>% filter(starttime >=as.Date("2021-07-08"))
  ...
  hfc_constr$id_dur <- with(hfc_constr, round(as.numeric(difftime(end_id, id_start, units = "mins"))), 2)
